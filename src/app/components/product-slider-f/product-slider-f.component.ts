...
@Component({
  selector: 'app-product-slider-f',
  templateUrl: './product-slider-f.component.html',
  styleUrls: ['./product-slider-f.component.css']
})
export class ProductSliderFComponent implements OnInit, AfterViewInit {
  @ViewChild('slider') sliderRef!: ElementRef;

  FemaleProduts: ProductResponse[] = [];
  ...
  ngOnInit(): void {
    this.productService.getAllProducts().subscribe(
      products => {
        this.FemaleProduts = products.filter(product => product.sexe === 'FEMALE');        this.updateItemsToShow();
  ...
  updateDisplayProducts() {
    const totalProducts = this.FemaleProduts.length;
    this.displayProducts = [
      ...this.FemaleProduts.slice(-this.itemsToShow),
      ...this.FemaleProduts,
      ...this.FemaleProduts.slice(0, this.itemsToShow)
    ];
  }
  ...
  private updateSliderPosition(animate: boolean): void {
    const sliderElement = this.sliderRef.nativeElement;
    const totalItems = this.FemaleProduts.length;
  ...
  onMouseMove(event: MouseEvent): void {
    ...
    const totalWidth = this.itemWidth * this.FemaleProduts.length;
