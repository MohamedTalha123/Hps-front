...
export class ProductSliderComponent implements OnInit, AfterViewInit {
  ...
  updateDisplayProducts() {
    ...
    this.displayProducts = [
      ...this.casioProducts.slice(-this.itemsToShow),
      ...this.casioProducts,
      ...this.casioProducts.slice(0, this.itemsToShow)
    ];
  }
  ...
  private updateSliderPosition(animate: boolean): void {
    const sliderElement = this.sliderRef.nativeElement;
    const totalItems = this.casioProducts.length;
  ...
  onMouseMove(event: MouseEvent): void {
    ...
    const totalWidth = this.itemWidth * this.casioProducts.length;
